...
if [ "$answer" != "${answer#[Yy]}" ]; then
	...
	source "$scriptpath/.macos" --no-restart
fi
...
if [ "$answer" != "${answer#[Yy]}" ]; then
	if [[ -d ~/.config/nvim ]]; then
		printf "~/.config/nvim already exists, skipping.\n"
	else
...
if [ "$answer" != "${answer#[Yy]}" ]; then
	if ! mas account &>/dev/null; then
		printf "Not signed into App Store, skipping.\n"
	else
...
if [ "$answer" != "${answer#[Yy]}" ]; then
	...
	for docapp in "${dockappstoremove[@]}"; do
		dockutil --remove "$docapp" 2>/dev/null || true
	done

	printf "Adding dock apps\n"
	dockappstoadd=()
	dockappstoadd=("${dockappstoadd[@]}" "/Applications/Ghostty.app")
